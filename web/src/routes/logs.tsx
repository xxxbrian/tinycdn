  ...
  loader: ({ location }) =>
    withProtectedLoader(location, async () => {
      const [requests, audit] = await Promise.all([
        api.requestLogs({ period: "24h", limit: 20 }),
        api.auditLogs({ period: "7d", limit: 12 }),
      ]);
...
function LogsPage() {
  ...
  useEffect(() => {
    ...
    void Promise.all([
      ...
      api.auditLogs({ period: period === "1h" ? "24h" : period, limit: 12 }),
  ...
  return (
    <ConsoleShell>
      ...
      <div className="grid gap-6 px-4 pb-6 2xl:grid-cols-[minmax(0,1.5fr)_20rem] lg:px-6">
        ...
        <AuditLogList
          title="Audit events"
          description="Recent configuration and purge activity."
          items={audit.items}
        />
      </div>
    </ConsoleShell>
